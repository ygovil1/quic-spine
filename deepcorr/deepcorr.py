...
dataset = []
dataset_location = "/tigress/ygovil/deepcorr/dataset/"

print("Loading database...")

for name in all_runs:
    filename = dataset_location + name + "_tordata300.pickle"
...
if TRAINING:
    ...
    test_index = pickle.load(open("test_index300.pickle", "rb"))[:1000]
...
def generate_data(dataset, train_index, test_index, flow_size):
    ...
    for i in tqdm.tqdm(train_index):
        ...
        l2s[index, 1, :, 0] = (
            np.array(dataset[i]["there"][0]["->"][:flow_size]) * 1000.0
        )
        l2s[index, 2, :, 0] = (
            np.array(dataset[i]["there"][0]["<-"][:flow_size]) * 1000.0
        )
        ...
        l2s[index, 5, :, 0] = (
            np.array(dataset[i]["there"][1]["->"][:flow_size]) / 1000.0
        )
        l2s[index, 6, :, 0] = (
            np.array(dataset[i]["there"][1]["<-"][:flow_size]) / 1000.0
        )
        ...
        for idx in random_ordering:
            ...
            l2s[index, 0, :, 0] = (
                np.array(dataset[idx]["here"][0]["<-"][:flow_size]) * 1000.0
            )
            l2s[index, 1, :, 0] = (
                np.array(dataset[i]["there"][0]["->"][:flow_size]) * 1000.0
            )
            ...
            l2s[index, 4, :, 0] = (
                np.array(dataset[idx]["here"][1]["<-"][:flow_size]) / 1000.0
            )
            l2s[index, 5, :, 0] = (
                np.array(dataset[i]["there"][1]["->"][:flow_size]) / 1000.0
            )
    ...
    for i in tqdm.tqdm(test_index):
        ...
        l2s_test[index, 0, :, 0] = (
            np.array(dataset[i]["here"][0]["<-"][:flow_size]) * 1000.0
        )
        l2s_test[index, 1, :, 0] = (
            np.array(dataset[i]["there"][0]["->"][:flow_size]) * 1000.0
        )
        ...
        l2s_test[index, 4, :, 0] = (
            np.array(dataset[i]["here"][1]["<-"][:flow_size]) / 1000.0
        )
        l2s_test[index, 5, :, 0] = (
            np.array(dataset[i]["there"][1]["->"][:flow_size]) / 1000.0
        )
...
def model(flow_before, dropout_keep_prob):
    ...
    for l in range(len(flat_layers_after) - 1):
        ...
        if l < len(flat_layers_after) - 2:
            _x = tf.nn.dropout(
                tf.nn.relu(_x, name="relu_noise_flat_%d" % l),
...
def model_cnn(flow_before, dropout_keep_prob):
    ...
    for cnn_size in range(len(CNN_LAYERS)):
        ...
        _x = tf.nn.conv2d(
            last_layer, cnn_weights, strides=[1, 2, 2, 1], padding="VALID"
        )
...
if TRAINING:
    ...
    with graph.as_default():
        train_flow_before = tf.placeholder(
            tf.float32,
            shape=[batch_size, 8, flow_size, 1],
            name="flow_before_placeholder",
        )
        train_label = tf.placeholder(
            tf.float32, name="label_placeholder", shape=[batch_size, 1]
        )
        ...
        loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=y2, labels=train_label),
            name="loss_sigmoid",
        )
...
writer = tf.summary.FileWriter(
    "./logs/tf_log/noise_classifier/allcir_300_" + str(datetime.datetime.now()),
    graph=graph,
)
    ...
    with tf.Session(graph=graph) as session:
        ...
        for i in tqdm.tqdm(test_index):
            ...
            for j in test_index:
                ...
                l2s_test_all[index, 0, :, 0] = (
                    np.array(dataset[j]["here"][0]["<-"][:flow_size]) * 1000.0
                )
                l2s_test_all[index, 1, :, 0] = (
                    np.array(dataset[i]["there"][0]["->"][:flow_size]) * 1000.0
                )
                l2s_test_all[index, 2, :, 0] = (
                    np.array(dataset[i]["there"][0]["<-"][:flow_size]) * 1000.0
                )
                l2s_test_all[index, 3, :, 0] = (
                    np.array(dataset[j]["here"][0]["->"][:flow_size]) * 1000.0
                )

                l2s_test_all[index, 4, :, 0] = (
                    np.array(dataset[j]["here"][1]["<-"][:flow_size]) / 1000.0
                )
                l2s_test_all[index, 5, :, 0] = (
                    np.array(dataset[i]["there"][1]["->"][:flow_size]) / 1000.0
                )
                l2s_test_all[index, 6, :, 0] = (
                    np.array(dataset[i]["there"][1]["<-"][:flow_size]) / 1000.0
                )
                l2s_test_all[index, 7, :, 0] = (
                    np.array(dataset[j]["here"][1]["->"][:flow_size]) / 1000.0
                )
                l_ids.append((xi, xj))
                index += 1
